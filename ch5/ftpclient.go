...
// Строки, используемые пользовательским интерфейсом
const (
	uiDir  = "dir"
	uiCd   = "cd"
	uiPwd  = "pwd"
...
// Строки, используемые участниками сети
const (
	DIR = "DIR"
	CD  = "CD"
	PWD = "PWD"
...
func main() {
	...
	for {
		...
		// Разделить строку на команду + аргумент
		strs := strings.SplitN(line, " ", 2)
		// Декодировать пользовательский запрос
		switch strs[0] {
		case uiDir:
			dirRequest(conn)
		case uiCd:
...
func dirRequest(conn net.Conn) {
	...
	for {
		// Читать, пока не наткнемся на перенос строки
		n, _ := conn.Read(buf[0:])
